                        ...
                        <li class="mb-1">
                          ...
                          <div class="collapse show" id="getting-started-collapse">
                            ...
                            <ul class="list-unstyled fw-normal pb-1 small">
                                <li><a style="font-size: 17.2px; margin-bottom: 15px; color: #246bf4 !important; font-weight: 500;" href="/my/orders" class="d-inline-flex align-items-center rounded active text-black" aria-current="page"><div style="border-left:3px solid #ffffff00; margin-left: -25px; margin-right: 20px; height:20px; border-radius: 500px;"></div>
...
border: none;
            ...
            <div class="col-md-9">
                <table class="table">
                    <thead>
                      <tr style="color: #999999; font-weight: 400 !important;  border:none !important;">
                    ...
                    <tbody>
                    <?php
        $accounts = DB::query('SELECT * FROM shops_tokens WHERE user_id=:uid ORDER BY id DESC', array(':uid'=>isLoggedIn()));
        foreach ($accounts as $ac) {
          if ($ac['account_id'] === 'cheat1' || $ac['account_id'] === 'cheat2' || $ac['account_id'] === 'cheat3' || $ac['account_id'] === 'cheat4' || $ac['account_id'] === 'cheat5' || $ac['account_id'] === 'cheat6') {
            $data = DBA::query('SELECT * FROM accounts_cheat WHERE id=:id', array(':id'=>$ac['accountcr_id']))[0];
        } else if ($ac['account_id'] === 'csgo1' || $ac['account_id'] === 'csgo2' || $ac['account_id'] === 'csgo3' || $ac['account_id'] === 'csgo4') {
          $data = DBA::query('SELECT * FROM accounts_csgo WHERE id=:id', array(':id'=>$ac['accountcr_id']))[0];
        }
            if ($ac['account_id'] === '1') {
                ...
                ✅ New email only for Tarkov acc<br>
                ✅ You will get Email/ Email password / Tarkov password';
            } else if ($ac['account_id'] === 'cheat1') {
                $name = 'EFT 1 Day Cheat Subscription';
                $descp = '⭐️ ESP (Opponents)<br>
⭐️ ESP (Items)<br>
⭐️ NoRecoil<br>
⭐️ Big heads<br>';
              } else if ($ac['account_id'] === 'cheat3') {
                $name = 'EFT 7 Days Cheat Subscription';
                $descp = '⭐️ ESP (Opponents)<br>
⭐️ ESP (Items)<br>
⭐️ NoRecoil<br>
⭐️ Big heads<br>';
              } else if ($ac['account_id'] === 'cheat4') {
                $name = 'EFT 15 Day Cheat Subscription';
                $descp = '⭐️ ESP (Opponents)<br>
⭐️ ESP (Items)<br>
⭐️ NoRecoil<br>
⭐️ Big heads<br>';
              } else if ($ac['account_id'] === 'cheat5') {
                $name = 'EFT 30 Day Cheat Subscription';
                $descp = '⭐️ ESP (Opponents)<br>
⭐️ ESP (Items)<br>
⭐️ NoRecoil<br>
⭐️ Big heads<br>';
              } else if ($ac['account_id'] === 'cheat6') {
                $name = 'EFT Lifetime Cheat Subscription';
                $descp = '⭐️ ESP (Opponents)<br>
⭐️ ESP (Items)<br>
⭐️ NoRecoil<br>
⭐️ Big heads<br>';
} else if ($ac['account_id'] === 'frl1') {
                $name = 'Farlight 1 Day Cheat';
               
} else if ($ac['account_id'] === 'frl2') {
                $name = 'Farlight 7 Days Cheat';
              
} else if ($ac['account_id'] === 'frl3') {
                $name = 'Farlight 30 Days Cheat';
} else if ($ac['account_id'] === 'csgo1') {
                $name = 'CS2 1 Day Cheat';
               
} else if ($ac['account_id'] === 'csgo2') {
                $name = 'CS2 7 Days Cheat';
              
} else if ($ac['account_id'] === 'csgo3') {
                $name = 'CS2 30 Days Cheat';
} else if ($ac['account_id'] === 'csgo5') {
                $name = 'CS2 Lifetime Cheat';
} else if ($ac['account_id'] === 'ap1') {
                $name = 'Apex 1 Day Cheat';
               
} else if ($ac['account_id'] === 'ap2') {
                $name = 'Apex 3 Days Cheat';
              
} else if ($ac['account_id'] === 'ap3') {
                $name = 'Apex 7 Days Cheat';
} else if ($ac['account_id'] === 'ap4') {
                $name = 'Apex 30 Days Cheat';
} else if ($ac['account_id'] === 'ov1') {
                $name = 'Overwatch 2 1 Day Cheat';
               
} else if ($ac['account_id'] === 'ov2') {
                $name = 'Overwatch 2 7 Days Cheat';
              
} else if ($ac['account_id'] === 'ov3') {
                $name = 'Overwatch 2 30 Days Cheat';
} else if ($ac['account_id'] === 'ov4') {
                $name = 'Overwatch 2 Lifetime Cheat';
                } else if ($ac['account_id'] === 'valk1') {
                $name = 'Valkyrie EFT Cheat 1 Day';
                } else if ($ac['account_id'] === 'valk2') {
                $name = 'Valkyrie EFT Cheat 7 Days';
                } else if ($ac['account_id'] === 'valk3') {
                $name = 'Valkyrie EFT Cheat 30 Days';
                } else if ($ac['account_id'] === 'valk4') {
                $name = 'Valkyrie EFT Cheat Lifetime';
              } else if ($ac['account_id'] === 'update') {
                $name = 'Update';
               
              } else if ($ac['account_id'] === 'spoofer') {
                $name = 'Spoofer';
             ...
             } else if ($ac['account_id'] === 'spoofer1') {
                $name = 'Spoofer';
                $descp = '<br>⚡ Enjoy your games while staying undetected.<br> Bypass detections with the help from our HWID Spoofer ⚡<br><br>  
                ...
                ✅ Cleaning usn magazines<br>
                ✅ Clean up system token logs<br>';
             } else if ($ac['account_id'] === 'spoofer2') {
                $name = 'Spoofer';
                $descp = '<br>⚡ Enjoy your games while staying undetected.<br> Bypass detections with the help from our HWID Spoofer ⚡<br><br>  
            ...
            if ((int)$ac['accountcr_id'] === 10000013) {
              $key = 'Contact technical support to get an account!';
            } else {
              $key = $data['key_t'];
            }
            echo ' <tr style="color: #fff;  border:none;">
            <th scope="row">'.$name.'</th>
            <td>'.$date.'</td>
            <td><a href="/my/instruction?id='.$ac['account_id'].'">Instruction</a></td>
            <td><a style="color: #246bf4;">'.$key.'</a></td>
          </tr>';
