    ...
    if (password_verify($password, DB::query('SELECT password FROM users WHERE email=:username', array(':username'=>$username))[0]['password']) or password_verify($password, DB::query('SELECT password FROM users WHERE username=:username', array(':username'=>$username))[0]['password'])) {
            ...
            if (!$user_id) {
                ...
                if (isset($_GET['redirect'])) {
                  header("Location: https://{$_SERVER['SERVER_NAME']}".$_GET['redirect']);
                } else {
                  header("Location: https://{$_SERVER['SERVER_NAME']}/my");
                }
...
?>
  ...
  <div class="row align-items-center g-5 py-5">
    ...
    <div class="col-lg-6">
      ...
      <h1 style="color: #fff !important; font-size: 75px;" class="fw-bold text-body-emphasis lh-1 mb-3">Account <br>Log in</h1>
...
?>
        <form method="post">
            <div class="mb-3">
              ...
              <button name="loginaccount" class="loginbtn">Login</button>
              <p style="color: #aba5a7;" class="mt-5">Not having an account? <a href="reg" style="color: #246bf4;">Sign up</a></p>
              <p style="color: #aba5a7;" >Forgot password? <a href="recovery-password" style="color: #246bf4;">Restore</a></p>
        </form>
    </div>
  </div>
  </div>
