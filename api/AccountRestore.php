...
$data = DB::query('SELECT * FROM restr_tokens WHERE token=:token', array(':token'=>$_GET['token']))[0];
...
if ($_POST['newpasswd'] === $_POST['confpasswd'])
            ...
            DB::query('UPDATE users SET password=:password WHERE id=:id', array(':id'=>$data['user_id'], ':password'=>password_hash($_POST['newpasswd'], PASSWORD_BCRYPT)));
              DB::query('INSERT INTO login_tokens VALUES (\'0\', :token, :user_id)', array(':token'=>sha1($token), ':user_id'=>$data['user_id']));
 $cstrong = True;
...
?>
        <form method="post">
            <div class="mb-3">
                ...
                <input name="newpasswd" type="text" class="form-control" id="exampleFormControlInput1" placeholder="Name">
              </div>
              <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Confirm Password</label>
                <input name="confpasswd" type="text" class="form-control" id="exampleFormControlInput1" placeholder="Name">
              </div>
