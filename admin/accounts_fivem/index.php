...
<link rel="stylesheet" href="/static/css/root.admin.css">
    ...
    <body>
    <?php 
    if (isset($_GET["sort"])) { $ty = '/api/Admin/AccountsFiveM/List?sort='.$_GET['sort']; } else {$ty = '/api/Admin/AccountsFiveM/List'; }
    ?>
    <script>
            $(document).ready(function() {
                ...
                $('#reviewAdd').submit(function(e) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: '/api/Admin/AccountsFiveM/Add',
                        data: formData,
                        processData: false,
            contentType: false,
...
<?php
if (isset($_GET['sort'])) {
           $count = DBA::query("SELECT COUNT(*) FROM accounts_fivem WHERE t=:t AND account_buyed=0 ORDER BY id DESC", array(':t'=>$_GET['sort']))[0]['COUNT(*)'];
       } else {
           $count = DBA::query("SELECT COUNT(*) FROM accounts_fivem WHERE account_buyed=0 ORDER BY id DESC")[0]['COUNT(*)'];
       }
       ?>
       ...
       <?php
       if (isset($_GET['sort'])) {
           $reviews = DBA::query("SELECT * FROM accounts_fivem WHERE t=:t AND account_buyed=0 ORDER BY id DESC", array(':t'=>$_GET['sort']));
       } else {
           $reviews = DBA::query("SELECT * FROM accounts_fivem WHERE account_buyed=0 ORDER BY id DESC");
       }
        ...
        foreach ($reviews as $r) {
            ...
            <div class="card-body">
                Key: '.$r['key_t'].'<br>
                '.$r['t'].'<br>
                <a class="btn btn-danger" href="/api/Admin/AccountsFiveM/Delete?id='.$r['id'].'">Delete</a>
            </div>
