...
<div class="mb-3">
  ...
  <input type="text" name="uses" class="form-control" id="exampleFormControlInput1">
        ...
        <h1>Files</h1>
        <a style="margin-top: 20px; margin-left: 7px;" href="upload" class="price__card__button">Upload</a>
        </div>
        
       <div id="chatc">
       <?php
        $reviews = DB::query('SELECT * FROM files ORDER BY id DESC');
        foreach ($reviews as $r) {

            echo '<div style="background-color: #292929;" class="card mb-3">
            <div class="card-body">
                <a href="https://aworex.com'.$r['url'].'">https://aworex.com/'.$r['url'].'</a><br>
                '.$r['product'].'<br>
            </div>
        </div>';
        }
