    ...
    <script>
            $(document).ready(function() {
                ...
                $('#reviewAdd').submit(function(e) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: '/api/Admin/AccountsOverwatch/Add',
                        data: formData,
                        processData: false,
            contentType: false,
...
<?php
if (isset($_GET['sort'])) {
           $count = DBA::query("SELECT COUNT(*) FROM accounts_overwatch WHERE t=:t AND account_buyed=0 ORDER BY id DESC", array(':t'=>$_GET['sort']))[0]['COUNT(*)'];
       } else {
           $count = DBA::query("SELECT COUNT(*) FROM accounts_overwatch WHERE account_buyed=0 ORDER BY id DESC")[0]['COUNT(*)'];
       }
       ?>
        <div class="col-md-auto d-flex align-items-center">
        <h1>Accounts Overwatch</h1>
       ...
       <?php
       if (isset($_GET['sort'])) {
           $reviews = DBA::query("SELECT * FROM accounts_overwatch WHERE t=:t AND account_buyed=0 ORDER BY id DESC", array(':t'=>$_GET['sort']));
       } else {
           $reviews = DBA::query("SELECT * FROM accounts_overwatch WHERE account_buyed=0 ORDER BY id DESC");
       }
