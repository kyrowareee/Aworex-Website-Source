...
<link rel="stylesheet" href="/static/css/root.admin.css">
    ...
    <body>
    <?php 
    if (isset($_GET["sort"])) { $ty = '/api/Admin/AccountsArena/List?sort='.$_GET['sort']; } else {$ty = '/api/Admin/AccountsArena/List'; }
    ?>
    <script>
            $(document).ready(function() {
                ...
                $('#reviewAdd').submit(function(e) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: '/api/Admin/AccountsArena/Add',
                        data: formData,
                        processData: false,
            contentType: false,
...
<?php
if (isset($_GET['sort'])) {
           $count = DBA::query("SELECT COUNT(*) FROM accounts_cheata WHERE t=:t AND account_buyed=0 ORDER BY id DESC", array(':t'=>$_GET['sort']))[0]['COUNT(*)'];
       } else {
           $count = DBA::query("SELECT COUNT(*) FROM accounts_cheata WHERE account_buyed=0 ORDER BY id DESC")[0]['COUNT(*)'];
       }
       ?>
        <div class="col-md-auto d-flex align-items-center">
        <h1>Accounts Arena</h1>
