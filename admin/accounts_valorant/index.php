...
<div style="margin-bottom:125px;" class="form__div col-sm-8">
  ...
  <option value="3">30 Days</option>
        ...
        <button name="add" data-bs-dismiss="modal" type="submit"  style="margin-bottom: 20px; margin-right: 7px;" href="?sort=1day" class="btn btn-sm btn-primary">Add</button>
        <button name="add5" data-bs-dismiss="modal" type="submit" style="margin-bottom: 20px; margin-right: 7px;" href="?sort=7days" class="btn btn-sm btn-primary">Add 5</button>
        <button name="add10" data-bs-dismiss="modal" type="submit" style="margin-bottom: 20px; margin-right: 7px;" href="?sort=30days" class="btn btn-sm btn-primary">Add 10</button>
        </div>
      </div>
      </form>
            <script>
...
<?php
if (isset($_GET['sort'])) {
           $count = DBA::query("SELECT COUNT(*) FROM accounts_valorant WHERE t=:t AND account_buyed=0 ORDER BY id DESC", array(':t'=>$_GET['sort']))[0]['COUNT(*)'];
       } else {
           $count = DBA::query("SELECT COUNT(*) FROM accounts_valorant WHERE account_buyed=0 ORDER BY id DESC")[0]['COUNT(*)'];
       }
       ?>
        <div class="col-md-auto d-flex align-items-center">
        <h1>Accounts Valorant</h1>
       ...
       <?php
       if (isset($_GET['sort'])) {
           $reviews = DBA::query("SELECT * FROM accounts_valorant WHERE t=:t AND account_buyed=0 ORDER BY id DESC", array(':t'=>$_GET['sort']));
       } else {
           $reviews = DBA::query("SELECT * FROM accounts_valorant WHERE account_buyed=0 ORDER BY id DESC");
       }
        ...
        foreach ($reviews as $r) {
            ...
            <div class="card-body">
                Key: '.$r['key_t'].'<br>
                '.$r['t'].'<br>
                <a class="btn btn-danger" href="/api/Admin/AccountsValorant/Delete?id='.$r['id'].'">Delete</a>
            </div>
